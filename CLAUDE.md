...
- `pinch`: Real two-finger pinch-to-zoom, delivered as genuine kernel multi-touch through the Android emulator's gRPC bridge — so it drives anything on screen (React Native, native views, WebViews), not just RN surfaces. `direction:"out"` zooms in, `"in"` zooms out; `x`/`y` set the focal point in screenshot pixels; `scale` is the finger-separation ratio (large values chain automatically); `angle` picks the finger axis. Returns `verification.meaningful` like `swipe`. **Android emulators only** — physical Android devices have no such bridge and iOS needs a multi-touch HID helper that no released idb ships, so both return an explicit error rather than a partial result. It never fakes a zoom by calling app code: success means real fingers moved.

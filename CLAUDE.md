...
- `get_network_requests` / `search_network` / `get_request_details` / `clear_network`: Network request tracking with URL/method/status filtering. `get_network_requests(summary=true)` returns counts by method, status, and domain
    ...
    - **Credential headers are redacted to scheme + length** (`authorization`, `cookie`, `x-api-key`, …); `verbose:true` prints them. A live bearer JWT runs ~1.5KB and was written into the transcript identically on every call, which is a secret-handling problem for anyone storing or sharing sessions, not just a size one.
